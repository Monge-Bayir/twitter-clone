...
async def create_user(name: str, api_key: str):
    ...
    return {"user": user.name, "api_key": user.api_key}
...
async def get_user_me(api_key: Annotated[Optional[str], Header()] = None):
    ...
    data = {
        ...
        "user": {
            ...
            "name": user.name,
            "followers": [
                {"id": x.follower.id, "name": x.follower.name} for x in user.followers
            ],
            "following": [
                {"id": x.followed.id, "name": x.followed.name} for x in user.following
            ],
        },
    }
    return data
